...
public class Highway implements Iterable<Vehicle> {
    ...
    public double getVelocityEastbound() {
        if(goEast == 0) return 0;
        double slowEast = Integer.MAX_VALUE;
        for(Vehicle v: contents){
            if(v.getDirection() == 1 && v.getVelocity()<slowEast)
                slowEast = v.getVelocity();
        }
        return slowEast;
    }
    ...
    public double getVelocityWestbound() {
        if(goWest == 0) return 0;
        double slowWest = Integer.MAX_VALUE;
        for(Vehicle v: contents){
            if(v.getDirection() == 2 && v.getVelocity()<slowWest)
                slowWest = v.getVelocity();
        }
        return slowWest;
    }
